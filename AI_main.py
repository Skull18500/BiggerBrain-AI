...
if __name__ == '__main__':
    ...
    while True:
        ...
        if user_input.lower() == "quit":
        ...
        elif user_input.lower() == "train":#if inputs 'train'
        
            batches = t_u.make_batches_fast(filename=trainfilename, chunk_size=chunksize, 
                                 batch_size=batchsize, max_batches=maxbatches)
        
            model.trainingloop(dataset = train_ds, epochs=epochs, lr=train_lr,  batchsize=batchsize)#TODO: implement profiling.
            ...
            model.trainingloop(batches, epochs=epochs, lr=lr, subset_fraction=subsetfraction, batchsize=batchsize)#TODO: implement profiling.
            ...
            with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA]) as prof:
                with torch.no_grad():
                    model._orig_mod.forward(test_input, iter=2, is_training=True)
            ...
            t_u.prepare_data_fast(BASE_DIR, bin)
            ...
            test_input = torch.randint(0, t_u.enc.n_vocab, (48, 256)).to(device)
            ...
            for i in range(10):
                with torch.amp.autocast('cuda'):
                    logits, iters = model._orig_mod.forward(test_input, iter=3, is_training=True)
            torch.cuda.synchronize()
            elapsed = (time.time() - start) / 10
            print(f"Average forward pass: {elapsed*1000:.0f}ms")
            print(f"Estimated epoch time with 268 batches: {elapsed*268/60:.1f} minutes")
        else:
